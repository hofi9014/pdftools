...
| SEC-017 | **(Audyt kodu, 2026-09-15, finding B1)** Filtr CSRF w `proxy.ts` porównywał `origin.startsWith(allowed)` — domena w pełni kontrolowana przez atakującego, np. `https://optimapdf.com.evil.com`, też zaczyna się od dozwolonego originu, więc przechodziła filtr | Parsowanie nagłówka (`Origin` lub pełny URL z `Referer`) przez `new URL(...).origin` i dokładne porównanie zbioru `ALLOWED_ORIGINS`, zamiast dopasowania prefiksowego. Dowód: `tests/proxy-csrf.mts` (`npm run test:proxy-csrf`) — test uruchomiony NAJPIERW przeciwko niezałatanemu kodowi, potwierdził 3 warianty realnego obejścia (status 200 zamiast 403), potem przeciwko poprawce — wszystkie 8/8 asercji PASS, w tym zachowanie SEC-004 (brak nagłówków → 403) i fallback przez `Referer` nienaruszone |
