...
const content = {
  ...
  zh: {
    ...
    sections: [
      {
        h: '1. 我们作为数据控制者',
        p: '您个人数据的控制者是Leszek Hofman，Dąbrówka Nowa，波兰（optimapdf.com）。作为数字服务提供商，我们仅在提供在线PDF工具所需的范围内处理数据。有关数据保护的所有问题，请通过以下电子邮件联系我们：kontakt@optimapdf.com。',
      },
      {
        h: '2. 我们处理哪些个人数据及其目的',
        p: 'OptimaPDF旨在最大限度地减少数据收集。我们仅处理以下数据：',
        items: [
          'IP地址——用于确保安全和防止滥用（正当利益，GDPR第6条第1款(f)项）。IP地址在服务器日志中最多保存7天。',
          '用户上传的文件——大多数工具完全在浏览器中处理文件（WebAssembly/JavaScript）。这些文件不会发送到服务器，也永远不会离开您的设备。需要外部库的工具（压缩、OCR、格式转换）除外，在这些工具中，文件会在服务器内存中临时处理，并在操作完成后立即删除——通常在几秒内。',
          '主题和语言偏好——仅存储在您浏览器的localStorage中。它们不会发送到服务器或与第三方共享。',
        ],
      },
      {
        h: '3. 处理的法律依据',
        p: '我们根据以下依据处理您的数据：',
        items: [
          'GDPR第6条第1款(b)项（合同履行的必要性）——根据您的要求提供PDF工具。',
          'GDPR第6条第1款(f)项（正当利益）——确保服务安全、防止滥用以及对网站运营的技术分析。',
          'GDPR第6条第1款(a)项（同意）——针对需要将内容发送到外部API（OpenRouter）的AI功能。使用AI需要通过接受条款获得明确同意。',
        ],
      },
      {
        h: '4. 我们保留数据的时间',
        ...
        items: [
          '服务器日志中的IP地址：最多7天。',
          '发送到客户端工具的文件：不存储——关闭浏览器标签页或刷新页面时从内存中删除。',
          '发送到服务器端工具的文件（压缩、OCR、转换）：处理完成后立即删除（通常在几秒内）。',
          'localStorage数据：保留至用户手动删除或浏览器缓存被清除。',
        ],
      },
      {
        h: '5. 我们与谁共享数据',
        ...
        items: [
          'OpenRouter.ai——使用AI功能时（AI Chat、AI Summary、AI Translate）。我们仅发送从PDF中提取的文本，不包含用户识别数据。OpenRouter遵守GDPR并具有适当的保障措施。',
          '托管服务提供商——我们的网站由外部托管服务商托管，该服务商可能存储匿名服务器日志（IP地址、浏览器类型），最多7天。该提供商持有ISO 27001认证并符合GDPR要求。',
        ],
      },
      {
        h: '6. 您在GDPR下的权利',
        p: '根据GDPR第15至22条，您享有以下权利：',
        items: [
          '知情权（第15条）——您可以询问我们是否处理您的数据并获取访问权。',
          '更正权（第16条）——您可以要求更正不准确的数据。',
          '删除权（第17条）——被遗忘权。',
          '限制处理权（第18条）。',
          '数据可携权（第20条）。',
          '反对权（第21条）。',
          '随时撤回同意的权利——不影响撤回同意前处理的合法性。',
        ],
      },
      {
        h: '7. 如何行使您的权利',
        p: '要行使上述任何权利，请发送电子邮件至：kontakt@optimapdf.com。我们将在30天内回复。我们不收取履行请求的费用，除非请求明显无理或过度（GDPR第12条第5款）。',
      },
      {
        h: '8. 投诉权',
        p: '如果您认为我们对您数据的处理违反了GDPR，您有权向波兰数据保护局局长（PUODO——Urząd Ochrony Danych Osobowych，ul. Stawki 2，00-193华沙，波兰）提出投诉。',
      },
      {
        h: '9. 向第三国的传输',
        p: '使用AI功能时，文本可能由位于美国的OpenRouter服务器处理。在此情况下，为确保充分的数据保护水平，将根据GDPR要求适用适当的保障措施，包括酌情适用标准合同条款。',
      },
      {
        h: '10. Cookies和本地存储',
        p: '我们不使用广告、跟踪或第三方分析Cookie。我们仅使用浏览器的localStorage来记住您的偏好（主题、语言）。这些数据存储在您的设备上，您可以随时通过清除浏览器的本地存储来删除。',
      },
      {
        h: '11. 本GDPR声明的变更',
        p: '我们保留修改本GDPR信息的权利。任何变更将通过更新本页顶部的日期来通知。我们建议您定期查看此内容。',
      },
    ],
  },
};
