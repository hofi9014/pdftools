...
import { pdfToHtml } from '@/lib/client-pdf';
import { useLocale } from '@/lib/locale-context';
import { t, type Locale } from '@/lib/i18n';
import { getToolIcon } from '@/lib/icons';

export default function PdfToHtml({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('html2pdf')}</div>
      ...
      <div className="tool-info-box rounded-2xl p-5 mb-6">
        ...
        <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
          <li>{t('page.pdf2html.info1', locale)}</li>
          <li>{t('page.pdf2html.info2', locale)}</li>
          <li>{t('page.pdf2html.info3', locale)}</li>
        </ul>
      </div>
      ...
      {success && <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded-xl p-4 mb-6">{t('page.pdf2html.success', locale)}</div>}
      {success && file && processedBlobRef.current && (
        <div className="flex justify-center mb-6">
          <CloudFileSaver blob={processedBlobRef.current} fileName={file.name.replace('.pdf', '.html')} />
        </div>
      )}
      ...
      <button onClick={handleConvert} disabled={loading || !file}
        ...
        {loading ? `⏳ ${t('btn.loading', locale)}` : `🌐 ${t('page.pdf2html.btn', locale)}`}
      </button>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 text-center">
        {[
          { icon: '🔒', text: t('feature.files_deleted', locale) },
          { icon: '⚡', text: t('page.pdf2html.fast_convert', locale) },
