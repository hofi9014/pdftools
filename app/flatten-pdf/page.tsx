...
import { flattenPDF, downloadPdf } from '@/lib/client-pdf';
...
export default function FlattenPdf({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('flatten')}</div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.flatten', locale)}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.flatten.desc', locale)}</p>
      </div>
      ...
      <div className="flex justify-center gap-2 mb-6">
        ...
        <div className="tool-info-box rounded-2xl p-5 mb-6">
          <h3 className="font-bold tool-heading mb-2">{t('page.flatten.info_title', locale)}</h3>
          <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
            <li>{t('page.flatten.info_1', locale)}</li>
            <li>{t('page.flatten.info_2', locale)}</li>
            <li>{t('page.flatten.info_3', locale)}</li>
          </ul>
        </div>
      ...
      <button onClick={handleFlatten} disabled={loading || !file}
        className={`w-full py-4 rounded-2xl font-bold text-lg transition
          ...
          {loading ? <span>⏳ {t('page.flatten.loading', locale)}</span> : <span>📋 {t('page.flatten.btn', locale)}</span>}
      </button>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 text-center">
        {[
            { icon: '🔒', text: t('feature.local_short', locale) },
            { icon: '⚡', text: t('feature.instant_result', locale) },
