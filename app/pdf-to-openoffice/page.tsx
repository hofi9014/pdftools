...
export default function PDFToOpenOffice({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('pdf2openoffice')}</div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.pdf2openoffice', locale)}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.pdf2openoffice.desc', locale)}</p>
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-4 mb-6">
        ...
        <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
          <li>{t('page.pdf2openoffice.info1', locale)}</li>
          <li>{t('page.pdf2openoffice.info2', locale)}</li>
          <li>{t('page.pdf2openoffice.info3', locale)}</li>
        </ul>
      </div>
      ...
      {success && file && processedBlobRef.current && (
        <div className="flex justify-center mb-6">
          <CloudFileSaver blob={processedBlobRef.current} fileName={file.name.replace('.pdf', '.odt')} />
        </div>
      )}
      ...
      <button onClick={handleConvert} disabled={loading || !file}
        ...
        {loading ? `⏳ ${t('btn.loading', locale)}` : `📄 ${t('page.pdf2openoffice.btn', locale)}`}
      </button>
    </div>
  );
}
