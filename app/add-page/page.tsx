...
import { addBlankPage, downloadPdf } from '@/lib/client-pdf';
...
export default function AddPage({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('addpage')}</div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.addpage', locale)}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.addpage.desc', locale)}</p>
      </div>
      ...
      <div className="tool-card rounded-2xl border p-6 mb-6">
        <h3 className="font-bold text-gray-700 dark:text-gray-300 mb-4">{t('page.addpage.where', locale)}</h3>
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            { value: 'start', label: t('page.addpage.start', locale) },
            { value: 'end', label: t('page.addpage.end', locale) },
            { value: 'custom', label: t('page.addpage.custom', locale) },
          ].map(opt => (
        ...
        {position === 'custom' && (
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('page.addpage.page_number', locale)}</label>
            <input type="number" min="1" value={customIndex} onChange={e => setCustomIndex(e.target.value)}
      ...
      {success && processedBlobRef.current && (
        <div className="flex justify-center mb-6">
          <CloudFileSaver blob={processedBlobRef.current} fileName="z-dodana-strona.pdf" />
        </div>
      )}
      ...
      <button onClick={handleSubmit} disabled={loading || !file}
        ...
        {loading ? `⏳ ${t('page.addpage.loading', locale)}` : `${t('page.addpage.btn', locale)}`}
      </button>
