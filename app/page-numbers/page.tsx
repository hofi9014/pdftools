...
export default function PageNumbers() {
  ...
  const handleAddNumbers = async () => {
    ...
    try {
      const [vPos, hPos] = position.split('-') as ['bottom' | 'top', 'left' | 'center' | 'right'];
      ...
      const blob = await downloadPdf(result, file.name.replace('.pdf', '_numerowany.pdf'));
  ...
  const positions = [
    { value: 'top-left', label: '↖ ' + t('page.pagenumbers.top_left', locale) },
    { value: 'top-center', label: '↑ ' + t('page.pagenumbers.top_center', locale) },
    { value: 'top-right', label: '↗ ' + t('page.pagenumbers.top_right', locale) },
    { value: 'bottom-left', label: '↙ ' + t('page.pagenumbers.bottom_left', locale) },
    { value: 'bottom-center', label: '↓ ' + t('page.pagenumbers.bottom_center', locale) },
    { value: 'bottom-right', label: '↘ ' + t('page.pagenumbers.bottom_right', locale) },
  ];

  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          <div className="text-6xl mb-4">{getToolIcon('pagenumbers')}</div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.pagenumbers', locale)}</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.pagenumbers.desc', locale)}</p>
        </div>
        ...
        <div className="tool-card rounded-2xl border p-6 mb-6 space-y-5">
          <h3 className="font-bold text-gray-700 dark:text-gray-300">{t('page.pagenumbers.settings', locale)}</h3>

          {/* Pozycja */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('page.pagenumbers.position_label', locale)}</label>
              ...
              {positions.map(pos => (
                <button key={pos.value} onClick={() => setPosition(pos.value)}
                  className={`p-3 rounded-xl border-2 text-center text-xs font-medium transition
                    ${position === pos.value ? 'border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 text-gray-700 dark:text-gray-300'}`}>
                  {pos.label}
                </button>
              ))}
            </div>
          ...
          {/* Numer startowy */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('page.pagenumbers.start_label', locale)}</label>
            <input
            ...
            />
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('page.pagenumbers.start_hint', locale)}</p>
          </div>

          {/* Rozmiar czcionki */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {t('page.pagenumbers.font_size_label', locale)}: <span className="!text-[var(--coffee-accent)] font-bold">{fontSize}pt</span>
            </label>
            <input type="range" min="8" max="24" step="2"
              value={fontSize} onChange={e => setFontSize(e.target.value)}
              className="w-full accent-blue-600 dark:accent-blue-400" />
            <div className="flex justify-between text-xs text-gray-400 dark:text-gray-500 mt-1">
              <span>{t('page.pagenumbers.font_size_min', locale)}</span>
              <span>{t('page.pagenumbers.font_size_max', locale)}</span>
            </div>
          </div>
        </div>
        ...
        {success && file && processedBlobRef.current && (
          <div className="flex justify-center mb-6">
            <CloudFileSaver blob={processedBlobRef.current} fileName={file.name.replace('.pdf', '_numerowany.pdf')} />
          </div>
        )}

        <button onClick={handleAddNumbers} disabled={loading || !file}
          ...
          {loading ? `⏳ ${t('page.pagenumbers.loading', locale)}` : `🔢 ${t('page.pagenumbers.button', locale)}`}
        </button>
