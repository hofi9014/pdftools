...
import { deletePages, downloadPdf } from '@/lib/client-pdf';
...
export default function DeletePages({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedPages.length === 0) { setError(t('page.delete.noselect', locale)); return; }
    setLoading(true);
    setError('');
    setSuccess(false);
    ...
    try {
      ...
      const blob = await downloadPdf(result, 'bez-stron.pdf');
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('delete')}</div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.delete', locale)}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.delete.desc', locale)}</p>
      </div>
      ...
      {success && processedBlobRef.current && (
        <div className="flex justify-center mb-6">
          <CloudFileSaver blob={processedBlobRef.current} fileName="bez-stron.pdf" />
        </div>
      )}
      ...
      <button onClick={handleSubmit} disabled={loading || !file || selectedPages.length === 0}
        ...
        {loading ? `⏳ ${t('page.delete.loading', locale)}` : `${t('page.delete.btn', locale)}`}
      </button>
