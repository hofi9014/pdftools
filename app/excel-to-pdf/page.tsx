...
export default function ExcelToPDF({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  const handleConvert = async () => {
    if (!file) { setError(t('page.excel2pdf.no_file', locale)); return; }
    setLoading(true);
    setError('');
    setSuccess(false);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          <div className="text-6xl mb-4">{getToolIcon('excel2pdf')}</div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.excel2pdf', locale)}</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.excel2pdf.desc', locale)}</p>
        </div>
      ...
      <div className="flex justify-center gap-2 mb-6">
        ...
        <div className="tool-info-box rounded-2xl p-5 mb-6">
          ...
          <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
            <li>{t('page.excel2pdf.info_1', locale)}</li>
            <li>{t('page.excel2pdf.info_2', locale)}</li>
            <li>{t('page.excel2pdf.info_3', locale)}</li>
          </ul>
        </div>
        ...
        {success && (
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded-xl p-4 mb-6">
            ✅ {t('page.excel2pdf.success', locale)}
          </div>
        )}
        ...
        <button onClick={handleConvert} disabled={loading || !file}
          ...
          {loading ? <span>⏳ {t('btn.loading', locale)}</span> : <span>📊 {t('page.excel2pdf.btn', locale)}</span>}
        </button>
