...
import { editMetadata, downloadPdf } from '@/lib/client-pdf';
...
export default function Metadata({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await editMetadata(file, { title, author, subject, keywords });
      const blob = await downloadPdf(result, 'z-metadanymi.pdf');
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('metadata')}</div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.metadata', locale)}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.metadata.desc', locale)}</p>
      </div>
      ...
      <div className="tool-card rounded-2xl border p-6 mb-6 space-y-4">
        {[
          { label: t('page.metadata.title', locale), value: title, set: setTitle, placeholder: t('page.metadata.title_placeholder', locale) },
          { label: t('page.metadata.author', locale), value: author, set: setAuthor, placeholder: t('page.metadata.author_placeholder', locale) },
          { label: t('page.metadata.subject', locale), value: subject, set: setSubject, placeholder: t('page.metadata.subject_placeholder', locale) },
          { label: t('page.metadata.keywords', locale), value: keywords, set: setKeywords, placeholder: t('page.metadata.keywords_placeholder', locale) },
        ].map((f) => (
          <div key={f.label}>
      ...
      <button onClick={handleSubmit} disabled={loading || !file}
        ...
        {loading ? `⏳ ${t('page.metadata.loading', locale)}` : `${t('page.metadata.btn', locale)}`}
      </button>
