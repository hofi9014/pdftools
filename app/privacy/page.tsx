...
const content = {
      ...
      },
      {
        ...
        sub: [
          ...
          },
          {
            ...
            items: [
              'Narzędzia klienckie (merge, split, rotate, watermark, page-numbers, crop-pdf, edit-pdf, sign-pdf, redact-pdf, flatten-pdf, delete-pages, extract-pages, reorder-pages, add-page, metadata, pdf-to-svg, pdf-to-epub, pdf-to-txt, fill-form, pdf-to-images, to-pdfa, compare-pdf, unlock-pdf, protect-pdf, compress-pdf, ocr-pdf, pdf-to-word, word-to-pdf, jpg-to-pdf, pdf-to-excel, excel-to-pdf, pdf-to-powerpoint, openoffice-to-pdf, pdf-to-openoffice, pdf-to-html, html-to-pdf) — plik jest przetwarzany w całości w przeglądarce za pomocą WebAssembly i JavaScript. Plik nie opuszcza Twojego urządzenia.',
              'Narzędzia serwerowe (url-to-pdf) — podany adres URL jest przetwarzany przez nasz serwer w celu pobrania i konwersji strony do PDF.',
            ],
          },
          {
            h: '3.2a Ograniczenia użycia funkcji AI',
            p: 'Funkcje AI (Chat AI, Streszczenie AI, Tłumacz AI) korzystają z zewnętrznego API OpenRouter za pośrednictwem naszego serwera — tekst wyodrębniony z PDF nie jest wysyłany bezpośrednio z Twojej przeglądarki do OpenRouter, lecz przechodzi przez nasz serwer, który przekazuje go dalej przy użyciu naszego współdzielonego klucza API.',
            items: [
              'Aby zapobiec nadużyciom, stosujemy limit zapytań (15 dziennie na adres IP). W tym celu Twój adres IP jest tymczasowo przechowywany w pamięci serwera przez maksymalnie 24 godziny, po czym jest usuwany. Adres IP nie jest przekazywany do OpenRouter ani żadnej stronie trzeciej — służy wyłącznie do lokalnego liczenia zapytań na naszym serwerze.',
              'Treść przesyłanego tekstu nie jest zapisywana na dysku ani wykorzystywana do trenowania modeli.',
            ],
          },
          {
      ...
      },
      {
        ...
        sub: [
          ...
          },
          {
            ...
            items: [
              'Client-side tools (merge, split, rotate, watermark, page-numbers, crop-pdf, edit-pdf, sign-pdf, redact-pdf, flatten-pdf, delete-pages, extract-pages, reorder-pages, add-page, metadata, pdf-to-svg, pdf-to-epub, pdf-to-txt, fill-form, pdf-to-images, to-pdfa, compare-pdf, unlock-pdf, protect-pdf, compress-pdf, ocr-pdf, pdf-to-word, word-to-pdf, jpg-to-pdf, pdf-to-excel, excel-to-pdf, pdf-to-powerpoint, openoffice-to-pdf, pdf-to-openoffice, pdf-to-html, html-to-pdf) — the file is processed entirely in the browser using WebAssembly and JavaScript. The file never leaves your device.',
              'Server-side tools (url-to-pdf) — the provided URL is processed by our server in order to fetch and convert the page into a PDF.',
            ],
          },
          {
            h: '3.2a AI feature usage limits',
            p: 'AI features (AI Chat, AI Summary, AI Translate) use the external OpenRouter API through our server — text extracted from your PDF is not sent directly from your browser to OpenRouter, but passes through our server, which forwards it using our shared API key.',
            items: [
              'To prevent abuse, we apply a rate limit (15 requests per day per IP address). For this purpose, your IP address is temporarily stored in server memory for up to 24 hours, after which it is deleted. Your IP address is not shared with OpenRouter or any third party — it is used solely for local request counting on our server.',
              'The submitted text content is not saved to disk or used for model training.',
            ],
          },
          {
      ...
      },
      {
        ...
        sub: [
          ...
          },
          {
            ...
            items: [
              'Ferramentas do lado do cliente (merge, split, rotate, watermark, page-numbers, crop-pdf, edit-pdf, sign-pdf, redact-pdf, flatten-pdf, delete-pages, extract-pages, reorder-pages, add-page, metadata, pdf-to-svg, pdf-to-epub, pdf-to-txt, fill-form, pdf-to-images, to-pdfa, compare-pdf, unlock-pdf, protect-pdf, compress-pdf, ocr-pdf, pdf-to-word, word-to-pdf, jpg-to-pdf, pdf-to-excel, excel-to-pdf, pdf-to-powerpoint, openoffice-to-pdf, pdf-to-openoffice, pdf-to-html, html-to-pdf) — o ficheiro é processado integralmente no navegador utilizando WebAssembly e JavaScript. O ficheiro nunca sai do seu dispositivo.',
              'Ferramentas do lado do servidor (url-to-pdf) — o URL fornecido é processado pelo nosso servidor para obter e converter a página em PDF.',
            ],
          },
          {
            h: '3.2a Limites de utilização das funções de IA',
            p: 'As funções de IA (AI Chat, AI Summary, AI Translate) utilizam a API externa da OpenRouter através do nosso servidor — o texto extraído do PDF não é enviado diretamente do seu navegador para a OpenRouter, mas passa pelo nosso servidor, que o reencaminha utilizando a nossa chave de API partilhada.',
            items: [
              'Para prevenir abusos, aplicamos um limite de pedidos (15 por dia e por endereço IP). Para este efeito, o seu endereço IP é armazenado temporariamente na memória do servidor por um máximo de 24 horas, após o que é eliminado. O seu endereço IP não é partilhado com a OpenRouter nem com terceiros — é utilizado exclusivamente para a contagem local de pedidos no nosso servidor.',
              'O conteúdo do texto submetido não é guardado em disco nem utilizado para treino de modelos.',
            ],
          },
          {
      ...
      },
      {
        ...
        sub: [
          ...
          },
          {
            ...
            items: [
              'Klientsideverktøy (merge, split, rotate, watermark, page-numbers, crop-pdf, edit-pdf, sign-pdf, redact-pdf, flatten-pdf, delete-pages, extract-pages, reorder-pages, add-page, metadata, pdf-to-svg, pdf-to-epub, pdf-to-txt, fill-form, pdf-to-images, to-pdfa, compare-pdf, unlock-pdf, protect-pdf, compress-pdf, ocr-pdf, pdf-to-word, word-to-pdf, jpg-to-pdf, pdf-to-excel, excel-to-pdf, pdf-to-powerpoint, openoffice-to-pdf, pdf-to-openoffice, pdf-to-html, html-to-pdf) — filen behandles fullstendig i nettleseren ved bruk av WebAssembly og JavaScript. Filen forlater aldri enheten din.',
              'Serversideverktøy (url-to-pdf) — den oppgitte URL-en behandles av vår server for å hente og konvertere siden til PDF.',
            ],
          },
          {
            h: '3.2a Bruksbegrensninger for AI-funksjoner',
            p: 'AI-funksjoner (AI Chat, AI Summary, AI Translate) bruker det eksterne OpenRouter-APIet via vår server — tekst utvunnet fra PDFen sendes ikke direkte fra nettleseren din til OpenRouter, men går gjennom vår server, som videresender den ved hjelp av vår delte API-nøkkel.',
            items: [
              'For å forhindre misbruk bruker vi en forespørselsgrense (15 per dag per IP-adresse). Til dette formålet lagres IP-adressen din midlertidig i serverens minne i maksimalt 24 timer, hvorpå den slettes. IP-adressen din deles ikke med OpenRouter eller tredjeparter — den brukes utelukkende til lokal telling av forespørsler på vår server.',
              'Det innsendte tekstinnholdet lagres ikke på disk og brukes ikke til modelltrening.',
            ],
          },
          {
      ...
      },
      {
        ...
        sub: [
          ...
          },
          {
            ...
            items: [
              'Outils côté client (merge, split, rotate, watermark, page-numbers, crop-pdf, edit-pdf, sign-pdf, redact-pdf, flatten-pdf, delete-pages, extract-pages, reorder-pages, add-page, metadata, pdf-to-svg, pdf-to-epub, pdf-to-txt, fill-form, pdf-to-images, to-pdfa, compare-pdf, unlock-pdf, protect-pdf, compress-pdf, ocr-pdf, pdf-to-word, word-to-pdf, jpg-to-pdf, pdf-to-excel, excel-to-pdf, pdf-to-powerpoint, openoffice-to-pdf, pdf-to-openoffice, pdf-to-html, html-to-pdf) — le fichier est traité entièrement dans le navigateur à l\'aide de WebAssembly et JavaScript. Le fichier ne quitte jamais votre appareil.',
              'Outils côté serveur (url-to-pdf) — l\'URL fournie est traitée par notre serveur afin de récupérer la page et de la convertir en PDF.',
            ],
          },
          {
            h: '3.2a Limites d\'utilisation des fonctionnalités IA',
            p: 'Les fonctionnalités IA (AI Chat, AI Summary, AI Translate) utilisent l\'API externe OpenRouter par l\'intermédiaire de notre serveur — le texte extrait du PDF n\'est pas envoyé directement depuis votre navigateur vers OpenRouter, mais transite par notre serveur, qui le transmet à l\'aide de notre clé API partagée.',
            items: [
              'Afin de prévenir les abus, nous appliquons une limite de requêtes (15 par jour et par adresse IP). À cette fin, votre adresse IP est temporairement stockée dans la mémoire du serveur pendant 24 heures maximum, puis supprimée. Votre adresse IP n\'est partagée ni avec OpenRouter ni avec des tiers — elle sert uniquement au comptage local des requêtes sur notre serveur.',
              'Le contenu du texte transmis n\'est pas enregistré sur disque ni utilisé pour l\'entraînement de modèles.',
            ],
          },
          {
      ...
      },
      {
        ...
        sub: [
          ...
          },
          {
            ...
            items: [
              'Strumenti lato client (merge, split, rotate, watermark, page-numbers, crop-pdf, edit-pdf, sign-pdf, redact-pdf, flatten-pdf, delete-pages, extract-pages, reorder-pages, add-page, metadata, pdf-to-svg, pdf-to-epub, pdf-to-txt, fill-form, pdf-to-images, to-pdfa, compare-pdf, unlock-pdf, protect-pdf, compress-pdf, ocr-pdf, pdf-to-word, word-to-pdf, jpg-to-pdf, pdf-to-excel, excel-to-pdf, pdf-to-powerpoint, openoffice-to-pdf, pdf-to-openoffice, pdf-to-html, html-to-pdf) — il file viene elaborato interamente nel browser utilizzando WebAssembly e JavaScript. Il file non lascia mai il vostro dispositivo.',
              'Strumenti lato server (url-to-pdf) — l\'URL fornito viene elaborato dal nostro server per recuperare la pagina e convertirla in PDF.',
            ],
          },
          {
            h: '3.2a Limiti di utilizzo delle funzionalità AI',
            p: 'Le funzionalità AI (AI Chat, AI Summary, AI Translate) utilizzano l\'API esterna di OpenRouter tramite il nostro server — il testo estratto dal PDF non viene inviato direttamente dal vostro browser a OpenRouter, ma passa attraverso il nostro server, che lo inoltra utilizzando la nostra chiave API condivisa.',
            items: [
              'Per prevenire abusi, applichiamo un limite di richieste (15 al giorno per indirizzo IP). A tal fine, il vostro indirizzo IP viene memorizzato temporaneamente nella memoria del server per un massimo di 24 ore, dopodiché viene eliminato. Il vostro indirizzo IP non viene condiviso con OpenRouter né con terze parti — viene utilizzato esclusivamente per il conteggio locale delle richieste sul nostro server.',
              'Il contenuto del testo inviato non viene salvato su disco né utilizzato per l\'addestramento dei modelli.',
            ],
          },
          {
  ...
  ja: {
    ...
    sections: [
      {
        h: '1. 総則',
        p: '本プライバシーポリシーは、OptimaPDFウェブサイト（optimapdf.com）のユーザーの個人データの処理および保護に関する原則を定めるものです。OptimaPDFはプライバシーとデータのセキュリティを最重要視しています。すべてのツールはプライバシーバイデザインの原則に従って設計されています。デフォルトでは、お客様のファイルはブラウザでローカルに処理されます。',
      },
      {
        h: '2. データ管理者',
        p: 'お客様の個人データの管理者は、Leszek Hofman、Dąbrówka Nowa、ポーランドです。連絡先：kontakt@optimapdf.com。管理者はデータ保護責任者を任命していません。個人データ保護に関するすべての問題については、上記のメールアドレス直接ご連絡ください。',
      },
      {
        h: '3. データ収集の範囲と目的',
        sub: [
          {
            h: '3.1 技術データ',
            p: 'ウェブサイトの利用時に、以下の技術データが自動的に収集されます。IPアドレス、ブラウザの種類とバージョン、OS、画面解像度、概算の地理的位置（国レベル）、訪問時刻、サイトでの滞在時間。このデータは匿名化され、統計的目的およびセキュリティ確保のためにのみ使用されます。',
          },
          {
          ...
          {
            h: '3.3 ユーザー設定',
            p: '選択されたテーマ（ダーク/ライト）と言語設定に関する情報は、ブラウザのlocalStorageに保存されます。サーバーに送信されることも、第三者と共有されることもありません。',
          },
        ],
      },
      {
        h: '4. 処理の法的根拠',
        items: [
          'GDPR第6条第1項（b）号 — 電子サービス契約の履行（PDFツールの提供）。',
          'GDPR第6条第1項（f）号 — 管理者の正当な利益（セキュリティの確保、不正利用の防止、技術的分析）。',
          'GDPR第6条第1項（a）号 — ユーザーの同意（AI機能について）。同意はいつでも撤回できます。',
        ],
      },
      {
        h: '5. データの受取者',
        p: 'データは以下のカテゴリの受取者に転送される場合があります。',
        items: [
          'Google LLC（Google Analytics） — お客様の同意に基づく匿名化されたトラフィック統計。',
          '当社の委託先の処理者（ホスティング、OpenRouter.ai） — データ処理契約に基づく。',
          '法令に基づく権限を持つ当局 — 法律で定められた場合のみ。',
        ],
      },
      {
        h: '6. EEA外へのデータ移転',
        p: 'AI機能の使用時、テキストは米国のOpenRouterサーバーで処理される場合があります。その場合、GDPRの要件に従い、標準契約条項を含む適切な保護措置が適用され、適切なレベルのデータ保護が確保されます。その他の場合、データは第三国に移転されません。',
      },
      {
        h: '7. データの保存期間',
        p: 'データは以下の期間保存されます。',
        items: [
          'サーバーログ（IPアドレス、User-Agent）：最大7日間。',
          'クライアント側ツールにアップロードされたファイル：保存されません。ページ更新時にメモリから削除されます。',
          'サーバー側ツールにアップロードされたファイル：処理完了後に即座に削除されます。',
      ...
      },
      {
        ...
        items: [
          'データへのアクセス（GDPR第15条）。',
          'データの訂正（GDPR第16条）。',
          'データの消去（GDPR第17条） — 忘れられる権利。',
          '処理の制限（GDPR第18条）。',
          'データポータビリティ（GDPR第20条）。',
          '処理への異議申立て（GDPR第21条）。',
          'いつでも同意を撤回できます。撤回前の同意に基づく処理の適法性に影響を及ぼしません。',
          '個人データ保護局長官（PUODO — Urząd Ochrony Danych Osobowych）、ul. Stawki 2、00-193 ワルシャワ、ポーランドに苦情を申し立てる権利。',
        ],
      },
      {
        h: '9. データのセキュリティ',
        p: '当社は以下のセキュリティ対策を講じています。',
        items: [
          'TLS/SSL暗号化 — ブラウザとサーバー間のすべての通信が暗号化されます。',
          'Content Security Policy（CSP） — 信頼できないスクリプトの実行能力を制限します。',
          'RAMのみでの処理 — ファイルはサーバーのディスクに書き込まれません。',
          '自動削除 — 操作完了後にファイルは即座に削除されます。',
          'ファイルサイズ制限 — 最大100MB。',
          'ファイルタイプの検証 — 処理前にファイル署名（マジックバイト）を確認します。',
          'ログイン不要 — 登録またはログインは必要ありません。',
        ],
      },
      {
        h: '10. Google Analyticsと分析の同意',
        p: 'このウェブサイトは、匿名のトラフィック分析のためにGoogle Analytics（Google LLC、USA）を使用しています。Google Analyticsは、訪問数、サイトでの滞在時間、ブラウザの種類、概算の位置情報（国レベル）、トラフィックのソースなど、集計された統計データを収集します。データは匿名化されています（anonymize_ip: true）。',
        items: [
          '初回訪問時に同意バナーが表示されます。Google Analyticsは「承認」をクリックした後にのみ有効になります。',
          'ブラウザのlocalStorageから「cookie-consent」のエントリを削除することで、いつでも同意を撤回できます。',
          'Google Analyticsは広告ターゲティングや行動プロファイリングには使用されません。',
          'localStorageのデータ（テーマ、言語、同意設定）は完全にユーザーの管理下にあり、いつでも削除できます。',
        ],
      },
      {
        h: '11. AI機能とOpenRouter',
        p: 'AI機能（AI Chat、AI Summary、AI Translate）を使用するには、ご自身のOpenRouter APIキーが必要です。このキーはブラウザのlocalStorageにのみ保存され、ウェブサイト管理者と共有されることはありません。APIキーはOpenRouter APIとの通信にのみ使用されます。当社はお客様のAPIキーおよびOpenRouterに送信されたクエリの内容にアクセスすることはできません。',
      },
      {
        h: '12. 雑則',
        p: '当社は、本プライバシーポリシーに変更を加える権利を留保します。変更がある場合は、本ページの上部の日付を更新してお知らせします。プライバシーポリシーに関するご質問は、kontakt@optimapdf.com までお問い合わせください。',
      },
    ],
  },
  tr: {
