...
export default function PDFToWord({ locale: forcedLocale }: { locale?: Locale } = {}) {
  ...
  const handleConvert = async () => {
    if (files.length === 0) { setError(t('page.word.no_files', locale)); return; }
    setLoading(true);
    setError('');
    setSuccess(false);
    setProgress(0);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <div className="text-6xl mb-4">{getToolIcon('word')}</div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tool-heading mb-3">{t('tool.word', locale)}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base md:text-lg">{t('page.word.desc', locale)}</p>
      </div>
      ...
      <div className="tool-info-box rounded-2xl p-5 mb-6">
        ...
        <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
          <li>{t('page.word.info1', locale)}</li>
          <li>{t('page.word.info2', locale)}</li>
          <li>{t('page.word.info3', locale)}</li>
        </ul>
      </div>
      ...
      <button onClick={handleConvert} disabled={loading || files.length === 0}
        ...
        {loading ? `⏳ ${t('page.word.loading', locale)} ${progress}/${files.length}` : `📝 ${t('page.word.btn', locale)} (${files.length})`}
      </button>
